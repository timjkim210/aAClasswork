...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
      actors.name = 'John Travolta' AND castings.ord = 1
    GROUP BY
      yr
    HAVING
      COUNT(*) >= 2
...
def andrews_films_and_leads
  ...
  #start with a subquery returning all of the movie titles Julie Andrews played in
  #list the film title and leading actor, actor and c
  execute(<<-SQL)
  SELECT
    movies.title, actors.name
  FROM
